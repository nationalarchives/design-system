...
const now = new Date(),
  ...
  iso8601DateString = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}-${String(now.getUTCDate()).padStart(2, "0")}`,
  logoAdornmentsCss = document.getElementById("logo-adornments-css");
...
const setTheme = (theme) => {
  ...
  cookies = new window.TNAFrontend.Cookies();
...
const resizeExampleIframe = ($iframe) => {
    ...
    const height = Math.ceil($body.getBoundingClientRect().height),
      extraHeight = $iframe.classList.contains(
